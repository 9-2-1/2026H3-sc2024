...
输出（按交付模块各出一份，归属见 cases_spec.MODULE1_IDS）：
    ...
    项目根目录\\附录1：测试用例清单（模块二）.xlsx   22 条，AI 扩展生成的进阶用例
...
OVERVIEW_1 = (
    ...
    '共 34 条用例，全部由小组人工设计并自动化执行（本阶段按要求不使用 AI 工具生成用例），'
    '覆盖等价类划分、边界值分析与场景法三种方法。用例只取材于对外可观测的行为——'
    '输入解析、接口契约、回合重置、评分判定、避障姿态分档代表值、异常安全、动作输出、'
    '数值健壮性与端到端抓取回合；每条用例的标题、输入与预期结果均可直接阅读判断，'
...
OVERVIEW_2 = (
    ...
    '共 22 条用例，是在模块一 34 条的基础上借助 AI 工具扩展生成的：'
    '由 AI 提出用例方向、构造输入观测、推演预期值并生成检查函数，'
    '小组逐条核对测试预言，剔除与 URDF / env.py 契约不符的臆测。'
    '用例集中在模块一未覆盖的深水区——运动学正解与连杆姿态、末端夹爪中心模型、'
    '距离度量的代数性质、避障判定表的全部阈值边界点、键盘微调通路的状态覆盖、'
    '观测入参的别名隔离、避障姿态的关节限位可实施性、'
    '逆解数值健壮性与关节限幅；判定每一条都需要 URDF 关节链、旋转矩阵'
    '或数值容差方面的专门知识。\n'
    '其中 TC-ALG-055（观测入参不得被就地修改）钉出了被测算法的一处真实缺陷：'
    'get_action 把观测切片按视图存入自身状态，下游键盘微调的就地写遂沿别名'
    '改写了调用方的观测。该缺陷由 AI 辅助白盒阅读定位、经人工实测确认并修复，'
    '详见《附录2：缺陷报告（模块二）》编号 006。\n'
...
MODULES = (
    ...
    },
    {
        ...
        'overview': OVERVIEW_2,
        'revision_note': 'initial 初稿完成：在模块一基础上由 AI 扩展生成 22 条进阶用例，'
